...
app.post('/ingest', async (req, res) => {
  ...
  await db.write();
  log('hospitalA.patient.created', { id: newP.id });
...
app.get('/patients', async (req, res) => {
  ...
  if (gender) {
    ...
    results = results.filter(p => (p.gender_canonical || '').toLowerCase() === g || (p.gender_local || '').toLowerCase() === g);
  }
  if (status) {
    results = results.filter(p => p.status === status);
  }
  if (since) {
    const t = Date.parse(since); if (!isNaN(t)) results = results.filter(p => Date.parse(p.createdAt) >= t);
  }
